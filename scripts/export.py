...
'''
https://huggingface.co/deepseek-ai/DeepSeek-R1-Distill-Qwen___1.5B

Download the DeepSeek R1 model from the above url.
'''

modelpath = '/home/gary/RK3576/rknn/05_llm/model/DeepSeek-R1-Distill-Qwen-1.5B'
llm = RKLLM()
...
ret = llm.load_huggingface(model=modelpath, model_lora = None, device='cpu', dtype="float16", custom_config=None, load_weight=True)
...
# Export rkllm model
ret = llm.export_rkllm(f"../model/{os.path.basename(modelpath)}_{quantized_dtype}_{target_platform}.rkllm")
if ret != 0:
